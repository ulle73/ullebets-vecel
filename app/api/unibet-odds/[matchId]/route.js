...
const toDecimalOdds = (fractional) => {
  ...
  const [numerator, denominator] = fractional.split(FRACTIONAL_SEPARATOR).map(Number);
...
export async function GET(_request, { params }) {
  ...
  const url = `${UNIBET_BASE_URL}/${encodeURIComponent(matchId)}.json?lang=sv_SE&market=SE&client_id=2&channel_id=1&includeParticipants=true`;
  ...
  try {
    ...
    if (!data?.betOffers || !Array.isArray(data.betOffers) || !data?.events || !data.events.length) {
    ...
    for (const offer of data.betOffers) {
      ...
      for (const outcome of offer?.outcomes ?? []) {
        ...
        const line = typeof outcome?.line === "number" ? (outcome.line / 1000).toFixed(3) : "x";
        const decimalValue = typeof outcome?.odds?.decimal === "number" ? outcome.odds.decimal : null;
        const decimalOdds = decimalValue != null ? decimalValue.toFixed(2) : toDecimalOdds(outcome?.oddsFractional);
