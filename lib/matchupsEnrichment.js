...
export async function loadMatchStatisticsFallback(matchId, rapidContext, cache, fallbackMeta = {}, logger = console) {
  if (typeof fallbackMeta === "function") {
    logger = fallbackMeta;
    fallbackMeta = {};
  }
  ...
  try {
    ...
    const matchDetails = buildMatchFromStatisticsPayload(statsResult?.statistics);
    const match = matchDetails
      ? {
          ...matchDetails,
          matchId: String(matchId),
          timestamp: fallbackMeta.timestamp ?? fallbackMeta.eventTimestampMs ?? null,
          homeTeamName: fallbackMeta.homeTeamName ?? null,
          awayTeamName: fallbackMeta.awayTeamName ?? null,
          leagueName: fallbackMeta.leagueName ?? null,
          matchDetails: {
