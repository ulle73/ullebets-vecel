import { resolveMatchupActualValue } from "../matchupsOutcome.js";
import { findTeamstatsMatchSelections } from "../teamstatsLookup.js";
import { isValidTrackedBet } from "./marketValidity.js";
...
export async function buildRankingFeedbackLookups(db, options = {}) {
  const snapshotCollection = options.snapshotCollection || "analysis-snapshots";
  const teamstatsCollection = options.teamstatsCollection || "teamstats";
  const limit = Math.min(Math.max(Number(options.limit) || 350, 50), 800);
  ...
  const matchMap = await findTeamstatsMatchSelections(db, uniqueMatchIds, {
    collectionName: teamstatsCollection,
  });
  ...
  for (const entry of dedupedEntries) {
    ...
    const actualValue = resolveMatchupActualValue(match, entry.bet)?.actualValue ?? null;
