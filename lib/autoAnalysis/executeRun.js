...
export async function executeAndPersistAutoAnalysisRun({
  ...
  await runMongoWithRetry("auto-analysis run upsert", () =>
    db.collection(AUTO_ANALYSIS_RUN_COLLECTION).updateOne(
      { runId },
      buildMongoUpsertDocument(
        {
          ...runDoc,
  ...
  if (result.candidates.length) {
    await runMongoWithRetry("auto-analysis candidate bulk upsert", () =>
      db.collection(AUTO_ANALYSIS_BET_COLLECTION).bulkWrite(
        result.candidates.map((candidate) => ({
          updateOne: {
            filter: { runId: candidate.runId, trackingKey: candidate.trackingKey },
            update: buildMongoUpsertDocument(
              {
                ...candidate,
                updatedAt: now,
              },
              { createdAtFallback: candidate.createdAt || now }
            ),
            upsert: true,
          },
        }))
  ...
  await runMongoWithRetry("auto-analysis snapshot upsert", () =>
    db.collection(SNAPSHOT_COLLECTION).updateOne(
      { runId },
      buildMongoUpsertDocument(snapshot, { createdAtFallback: now }),
      { upsert: true }
    )
  );
