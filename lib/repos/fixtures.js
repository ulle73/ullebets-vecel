 ...
 * Searches recent dates (today ± 7 days) for the match.
 * 
 * @param {string|number} matchId - Match ID to find
 * @param {Object} options - Options
 * @param {number} options.dayRange - Days to search before/after today (default: 7)
...
export async function getMatchById(matchId, options = {}) {
  ...
  try {
    ...
    // Generate date range (today ± dayRange days)
    ...
    // Search all dates for the matchId
    for (const dateStr of dates) {
      ...
      if (found) {
        log(`✓ getMatchById(${matchId}) found in ${dateStr} ${dt(t)}`);
        return found;
      }
    }
    
    log(`✗ getMatchById(${matchId}) not found in ±${dayRange} days ${dt(t)}`);
    return null;
...
/**
 * Get multiple matches by their IDs (batch fetch).
 * 
 * @param {Array<string|number>} matchIds - Array of match IDs
 * @param {Object} options - Options
 * @param {number} options.dayRange - Days to search (default: 7)
 * @returns {Promise<Array>} Array of found matches (may be partial)
 */
export async function getMatchesByIds(matchIds, options = {}) {
  ...
  try {
    const results = [];
    const remaining = new Set(matchIds.map(String));
    ...
    // Search each date until all IDs are found or dates exhausted
    for (const dateStr of dates) {
      ...
      for (const match of matches) {
        const mId = extractMatchId(match);
        if (mId && remaining.has(String(mId))) {
          results.push(match);
          remaining.delete(String(mId));
        }
      }
    }
    
    log(`✓ getMatchesByIds(${matchIds.length} IDs) found ${results.length} ${dt(t)}`);
    if (remaining.size > 0) {
      log(`  Missing IDs: ${Array.from(remaining).join(', ')}`);
    }
  ...
  if (FILE_FALLBACK) {
    try {
      ...
      if (fs.existsSync(fp)) {
        ...
        // const hydrated = await hydrateScoresFromTeamstats({
        //   matches: arr,
        //   client,
        //   date: dateStr,
        // });
        ...
        //         full: [
        //           {
        //             matches: JSON.parse(JSON.stringify(hydrated)),
        //             source: "fallback",
        //             updatedAt: new Date(),
        //           },
        //         ],
        ...
        //     { upsert: true }
        //   );
        //   log("getMatchesForDate:fallbackCached", {
        //     date: dateStr,
        //     count: hydrated.length,
        //     dur: dt(t),
        //   });
        // } catch (cacheErr) {
        //   log("getMatchesForDate:fallbackCacheError", {
        //     date: dateStr,
        //     err: String(cacheErr),
        //   });
        // }
