...
export async function writeSnapshot(params) {
  ...
  // Calculate horizonDays if matchDate is available
  let horizonDays = null;
  if (metadata.matchDate && date) {
    const matchTime = new Date(metadata.matchDate).getTime();
    const runTime = new Date(date).getTime();
    const diffTime = matchTime - runTime;
