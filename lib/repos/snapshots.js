...
import { runMongoWithRetry } from "../mongoUtils.js";
...
export async function readSnapshots(collection, id, options = {}) {
  ...
  try {
    ...
    const doc = await runMongoWithRetry("snapshot read", () => col.findOne({ _id: id }));
...
export async function readSnapshotDocument(collection, id) {
  ...
  try {
    ...
    return await runMongoWithRetry("snapshot document read", () => col.findOne({ _id: id }));
  } catch (error) {
...
export async function deleteOldSnapshots(collection, beforeDate) {
  ...
  try {
    ...
    const result = await runMongoWithRetry("snapshot cleanup", () =>
      col.deleteMany({
