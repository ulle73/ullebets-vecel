...
import { clientPromise } from "../db.js";
import { findTeamstatsMatchSelection } from "../teamstatsLookup.js";
import { performance } from "node:perf_hooks";
...
function mapMatchDoc(doc) {
  const f0 = Array.isArray(doc.full) ? (doc.full[0] ?? {}) : {};
  return mapMatchSnapshot(f0, doc._id);
}
...
export async function getMatch(matchId) {
  ...
  try {
    ...
    const selection = await findTeamstatsMatchSelection(db, matchId, { collectionName: COL });
    const found = !!selection?.match;
    const mapped = selection?.match ? mapMatchSnapshot(selection.match, matchId) : null;
    log("getMatch:done", { matchId: String(matchId), found, dur: dt(t) });
    return mapped;
  } catch (err) {
    log("getMatch:error", { matchId: String(matchId), err: String(err) });
...
export async function getIncidents(matchId) {
  ...
  try {
    ...
    const db = client.db(DB);
    const selection = await findTeamstatsMatchSelection(db, matchId, { collectionName: COL });
    const arr = Array.isArray(selection?.match?.incidents) ? selection.match.incidents : [];
    log("getIncidents:done", { matchId: String(matchId), count: arr.length, dur: dt(t) });
    return arr;
  } catch (err) {
    log("getIncidents:error", { matchId: String(matchId), err: String(err) });
...
export async function getStatistics(matchId) {
  ...
  try {
    ...
    const db = client.db(DB);
    const selection = await findTeamstatsMatchSelection(db, matchId, { collectionName: COL });
    const stats = selection?.match?.matchDetails?.statistics ?? selection?.match?.statistics;
    const arr = Array.isArray(stats) ? stats : [];
    log("getStatistics:done", { matchId: String(matchId), groups: arr.length, dur: dt(t) });
    return arr;
  } catch (err) {
...
const isDirect = Boolean(scriptPath) && (
  import.meta.url === pathToFileURL(scriptPath).href ||
  fileURLToPath(import.meta.url) === path.resolve(scriptPath)
);
