...
export async function getUnibetOddsForMatch(matchInfo, options = {}) {
  ...
  // Step 1: If eventId provided directly, use it
  const directEventId = extractUnibetEventId(matchInfo?.eventId);
  
  if (directEventId) {
    try {
      const oddsData = await fetchUnibetOdds(directEventId);
      const eventUrl = `${UNIBET_EVENT_BASE_URL.replace(/\/$/, '')}/${directEventId}`;
      ...
      console.error(`[unibet-engine] fetchUnibetOdds(${directEventId}) failed:`, error.message);
      throw error;
    }
  }
  
  // Step 2: Find event via findUnibetEventForMatch
  const searchInfo = {
