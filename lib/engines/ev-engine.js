 ...
 */
function resolvePrimaryEvValue(evDetails, statKey) {
  if (!evDetails) return null;


  // Fallback to normal priority order
  // Special case: LeagueAvg first for cornerKicks
  let preferredOrder;
  if (statKey === 'cornerKicks') {
    preferredOrder = [
      'evPctLeagueAvg',          // Prioritize LeagueAvg for cornerKicks
      'evPctWithMultiplier',
