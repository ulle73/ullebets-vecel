...
export function buildTeamRecentMatchesKey(match, side, options = {}) {
  if (!match) return null;
  const limit = Number.isFinite(options?.limit)
    ? Math.max(1, Math.min(10, Math.trunc(options.limit)))
    : 5;
  const normalizedSide = side === "away" ? "away" : "home";
  const teamPrefix = normalizedSide === "away" ? "away" : "home";
  const teamId = readMatchField(match, `${teamPrefix}TeamId`);
  const teamName = readMatchField(match, `${teamPrefix}TeamName`);

  const params = new URLSearchParams();
  params.append("limit", String(limit));

  const teamIdStr = toNumericIdString(teamId);
  const teamNameStr = toTrimmedString(teamName);
  ...
  return `/api/teamstats/recent?${params.toString()}`;
}
