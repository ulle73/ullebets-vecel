...
export function buildTeamProfileKey({
  ...
  const normalizedMatchType = toTrimmedString(matchType).toLowerCase();
  if (!normalizedMatchType) return null;

  const params = new URLSearchParams();
  const leagueIdStr = toNumericIdString(leagueId);
  const leagueNameStr = toTrimmedString(leagueName);
  ...
  params.append("matchType", normalizedMatchType);
