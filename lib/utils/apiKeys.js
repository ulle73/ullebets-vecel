...
export function buildTeamProfileKeyForMatch(match, side) {
  ...
  if (!matchId && process.env.NODE_ENV === "development") {
    // Debug aid: log when we fail to build a key so we can see the shape of the match object.
    // Avoid logging entire match to reduce noise; cherry-pick common id/name fields.
    console.warn("[buildTeamProfileKeyForMatch] missing matchId", {
      side: normalizedSide,
      leagueId,
      leagueName,
      teamId,
