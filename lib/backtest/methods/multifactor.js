...
export function computeMultifactorProjection({ base, leagueProjection }) {
  const components = {};
  let lambda = null;
  let totalWeight = 0;
  ...
  if (baseLambda != null && baseWeight > 0) {
    lambda = (lambda ?? 0) + baseLambda * baseWeight;
    totalWeight += baseWeight;
    components.base = { lambda: baseLambda, weight: baseWeight };
  }

  if (leagueLambda != null && leagueWeight > 0) {
    lambda = (lambda ?? 0) + leagueLambda * leagueWeight;
    totalWeight += leagueWeight;
    components.league = { lambda: leagueLambda, weight: leagueWeight };
  }
