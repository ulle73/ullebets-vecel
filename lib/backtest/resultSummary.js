...
function humanizeScope(scope, result) {
  if (scope === "home") return result?.bet?.homeTeam ? `Hemmalaget – ${result.bet.homeTeam}` : "Hemmalaget";
  if (scope === "away") return result?.bet?.awayTeam ? `Bortalaget – ${result.bet.awayTeam}` : "Bortalaget";
