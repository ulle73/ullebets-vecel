...
const PERIOD_MARKET_PRIORS = { ALL: 100, "1ST": 88, "2ND": 78 };
const SCOPE_MARKET_PRIORS = { total: 100, home: 87, away: 87 };
...
export const STRATEGY_PROFILES = {
  safe: {
    ...
    allowedStats: null,
    boosts: {},
    weights: { edge: 0.18, confidence: 0.28, consensus: 0.22, sample: 0.16, price: 0.08, market: 0.08, risk: 1.2 },
  },
  balanced: {
    id: "balanced",
    label: "Balans",
    ...
    allowedStats: null,
    boosts: {},
    weights: { edge: 0.28, confidence: 0.22, consensus: 0.18, sample: 0.12, price: 0.08, market: 0.12, risk: 1 },
  },
  aggressive: {
    id: "aggressive",
    label: "Aggressiv",
    ...
    allowedStats: null,
    boosts: {},
    weights: { edge: 0.42, confidence: 0.12, consensus: 0.1, sample: 0.08, price: 0.06, market: 0.08, risk: 0.72 },
  },
  corners: {
    id: "corners",
    label: "Hörnor",
    ...
    weights: { edge: 0.3, confidence: 0.18, consensus: 0.18, sample: 0.12, price: 0.08, market: 0.14, risk: 0.9 },
  },
  shots: {
    id: "shots",
    label: "Skott",
...
function buildPriceScore(odds) {
  ...
  if (odds > 3.4) return Math.round(clamp(30, raw, 75));
  return Math.round(clamp(35, raw, 100));
}

function buildMarketScore(result) {
  const statScore = STAT_MARKET_PRIORS[result?.bet?.statKey] || 60;
  const periodScore = PERIOD_MARKET_PRIORS[result?.bet?.period || "ALL"] || 80;
  const scopeScore = SCOPE_MARKET_PRIORS[result?.bet?.scope || "total"] || 80;
...
function buildEdgeScore(primaryEv) {
  ...
  return Math.round(clamp(0, 100 * (1 - Math.exp(-primaryEv / 7.5)), 100));
}
...
function buildLearningKeys(result) {
  ...
  return {
    ...
    leagueStatKey: `${normalizeKey(result?.leagueName || result?.bet?.leagueName || "unknown-league")}|${statKey}`,
  };
}

export function buildLearningAdjustment(result, learningProfile = null) {
  const lookups = learningProfile?.lookups;
  if (!lookups) return { adjustment: 0, confidencePct: 0, sources: [], proofReady: false, minBets: 0 };

  const keys = buildLearningKeys(result);
  const sourceSpecs = [
    { id: "stat", label: "Stat-historik", weight: 0.45, bucket: lookups?.stat?.[keys.statKey] },
    { id: "scope-period", label: "Scope/period-historik", weight: 0.2, bucket: lookups?.scopePeriod?.[keys.scopePeriodKey] },
    { id: "league-stat", label: "Liga + stat-historik", weight: 0.35, bucket: lookups?.leagueStat?.[keys.leagueStatKey] },
  ];

  const weightedSources = sourceSpecs
    .filter((spec) => spec.bucket && (Number(spec.bucket.bets) || 0) >= 8)
    .map((spec) => {
  ...
  if (!weightedSources.length) return { adjustment: 0, confidencePct: 0, sources: [], proofReady: false, minBets: 0 };
  ...
  const proofReady = confidencePct >= 35 && minBets >= 20;
...
function buildProofStatus(result, metrics, ranking, learning) {
  const modelCoverageReady = metrics.available >= 3;
  const sampleReady = metrics.sampleSize >= 8;
  const historicalReady = Boolean(learning?.proofReady);
  const proofScore = Math.round(
    clamp(0, metrics.confidenceScore * 0.5 + Math.min(metrics.sampleSize, 20) * 1.8 + (learning?.confidencePct || 0) * 0.18, 100)
  );
  const label = proofScore >= 78 ? "Bevisad" : proofScore >= 58 ? "OK underlag" : "Tunn data";
  const flags = [];
...
function buildRankingReasons({ edgeScore, confidenceScore, consensusScore, priceScore, marketScore, sampleScore, riskScore, bet, learning, proof }) {
  const reasons = [];
...
function buildRankingContext(result, learningProfile = null) {
  ...
  return {
    ...
    priceScore: buildPriceScore(Number(result?.bet?.odds)),
    marketScore: buildMarketScore(result),
...
export function enrichPositiveResult(result, learningProfile = null) {
  ...
  const rankReasons = buildRankingReasons({ ...ranking, riskScore, bet: enriched.bet, learning, proof });
  return {
    ...enriched,
    ...metrics,
    ranking,
...
export function matchesStrategyFilters(result, strategyId = "balanced") {
  ...
  if (strategy.id === "safe" && enriched.proof?.proofScore < 55) return false;
  return true;
}
...
export function scoreResultForStrategy(result, strategyId = "balanced", learningProfile = null) {
  ...
  const score =
    enriched.ranking.edgeScore * weights.edge +
    enriched.ranking.confidenceScore * weights.confidence +
    enriched.ranking.consensusScore * weights.consensus +
    enriched.ranking.sampleScore * weights.sample +
    enriched.ranking.priceScore * weights.price +
    enriched.ranking.marketScore * weights.market +
    statBoost +
    enriched.ranking.learningAdjustment +
    enriched.proof.proofScore * 0.05 -
