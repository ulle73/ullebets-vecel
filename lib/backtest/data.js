...
const TEAM_MATCH_CACHE = createCache({ ttlMs: 12 * 60 * 60 * 1000 }); // 12h
const TEAM_PROFILE_CACHE = createCache({ ttlMs: 12 * 60 * 60 * 1000 });

const log = (...args) => console.log("[backtest:data]", ...args);
...
async function queryTeamStatsDocument(filter, teamName, matchType) {
  ...
  log("queryTeamStatsDocument", { filter, teamName, matchType });
  ...
  if (doc) {
    ...
    log("teamstats match miss", { teamName, matchType });
  }
...
export async function fetchTeamMatches(teamName, matchType = "home") {
  ...
  if (cached) {
    log("fetchTeamMatches cache hit", { teamName, matchType, count: cached.length });
    return cached;
  }
  ...
  log("fetchTeamMatches cache set", { teamName, matchType, count: matches.length });
  return matches;
}
...
async function queryTeamProfile(teamName, matchType) {
  ...
  log("queryTeamProfile", { teamName, matchType, filter });
  ...
  if (doc) {
    ...
    log("teamprofile miss", { teamName, matchType });
  }
...
export async function fetchTeamProfile(teamName, matchType) {
  ...
  if (cached) {
    log("fetchTeamProfile cache hit", { teamName, matchType });
    return cached;
  }
...
export async function loadLeagueRankings() {
  log("loadLeagueRankings called – returning empty list (teamprofiles used instead)");
  return [];
}
