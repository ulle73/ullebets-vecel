...
const TEAM_MATCH_CACHE = createCache({ ttlMs: DAY_MS });
const TEAM_PROFILE_CACHE = createCache({ ttlMs: DAY_MS });
const LEAGUES_CACHE = createCache({ ttlMs: DAY_MS });
...
async function fetchLeaguesFromDatabase() {
  const client = await clientPromise;
  const db = client.db(process.env.MONGODB_DB || "app");
...
export async function loadLeaguesAndTeams() {
  const cacheKey = "leagues";
  const cached = LEAGUES_CACHE.get(cacheKey);
  if (cached) {
    logServerBacktestStep("Ligadata hämtades från cacheminnet.", {
      leagues: Object.keys(cached).length,
    });
    return cached;
  }

  const leagues = await fetchLeaguesFromDatabase();
  LEAGUES_CACHE.set(cacheKey, leagues);
