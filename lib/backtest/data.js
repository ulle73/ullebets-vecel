...
const TEAM_MATCH_CACHE = createCache({ ttlMs: TWENTY_FOUR_HOURS_MS });
const TEAM_PROFILE_CACHE = createCache({ ttlMs: TWENTY_FOUR_HOURS_MS });
const LEAGUES_CACHE = createCache({ ttlMs: TWENTY_FOUR_HOURS_MS });
...
export async function fetchLeaguesAndTeams() {
  const cacheKey = "leagues-and-teams";
  const cached = LEAGUES_CACHE.get(cacheKey);
  if (cached) {
    logServerBacktestStep("Ligatabeller hämtades från cacheminnet.");
    return cached;
  }
  ...
  if (!doc) {
    logServerBacktestStep("Ligatabeller saknas i databasen.");
    return null;
  }

  LEAGUES_CACHE.set(cacheKey, doc);
  logServerBacktestStep("Ligatabeller hämtades från databasen och cachades.");
  return doc;
}
