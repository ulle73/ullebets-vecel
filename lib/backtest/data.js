...
function sliceMatchesFromDoc(doc, limit) {
  const all = Array.isArray(doc?.full) ? doc.full : [];
...
async function loadTeamStatsDoc(teamName, matchType) {
  if (!teamName) return null;

  const normalizedMatchType = matchType === "away" ? "away" : "home";

  const primaryFilter = {
    "_importMeta.teamName": teamName,
    "_importMeta.teamRole": normalizedMatchType,
  };

  const fallbackFilter =
    normalizedMatchType === "home"
      ? { "full.0.homeTeamName": teamName }
      : { "full.0.awayTeamName": teamName };

  const doc =
    (await queryTeamStatsDocument(primaryFilter, teamName, normalizedMatchType)) ||
    (await queryTeamStatsDocument(fallbackFilter, teamName, normalizedMatchType));
...
export async function fetchTeamMatches(
  ...
  const doc = await loadTeamStatsDoc(teamName, matchType);
  const matches = sliceMatchesFromDoc(doc, Number.isFinite(limit) ? limit : undefined);
...
export async function fetchTeamMatchesWithMeta(
  teamName,
  matchType = "home",
  options = {}
) {
  ...
  if (!teamName) {
    return {
      ...
      meta: {
        sourceFile: null,
        teamName: null,
        teamRole: matchType === "away" ? "away" : "home",
        importedAt: null,
      },
    };
  }
  ...
  return {
    matches,
    meta: {
      sourceFile: doc?._importMeta?.sourceFile ?? null,
      teamName: doc?._importMeta?.teamName ?? null,
      teamRole: doc?._importMeta?.teamRole ?? (matchType === "away" ? "away" : "home"),
      importedAt: doc?._importMeta?.importedAt ?? null,
    },
  };
}
