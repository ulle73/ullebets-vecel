...
export async function calculateEVFromData(params, fetchedData) {
    const {
      ...
      const homeHistory = getMergedHistory(homeBundle, statProfileKey, parsedPeriod);
      const awayHistory = getMergedHistory(awayBundle, statProfileKey, parsedPeriod);
...
function getMergedHistory(bundle, statKey, periodKey) {
  const history = [];
  ...
  if (bundle?.home) extract(bundle.home);
  if (bundle?.away) extract(bundle.away);
