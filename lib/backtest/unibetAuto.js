...
let unibetLeagueConfigPromise = null;
...
function ensureUnibetLeagueConfigLoaded() {
  if (!unibetLeagueConfigPromise) {
    unibetLeagueConfigPromise = loadUnibetLeagueConfig().catch((err) => {
      unibetLeagueConfigPromise = null;
      throw err;
    });
  }
  return unibetLeagueConfigPromise;
}

// Start initial load but ignore failures (handled on demand later)
ensureUnibetLeagueConfigLoaded().catch(() => {});
...
// --- HÄMTA DATA ---
async function fetchListViewEvents(baseUrl, { forceRefresh = false } = {}) {
...
// --- HITTA MATCH ---
export async function findUnibetEventForMatch(matchInfo = {}, options = {}) {
  const { forceRefresh = false } = options || {};
  await ensureUnibetLeagueConfigLoaded();
  ...
  const events = await fetchListViewEvents(config.baseUrl, { forceRefresh });
  ...
  if (result.candidates.length === 0) {
    console.warn("Unibet auto: no match found", {
      ...
      looking_for: {
        homeNorm: context.matchHomeNorm,
        awayNorm: context.matchAwayNorm,
        home: canonicalHomeTeam,
        away: canonicalAwayTeam,
      },
      sample_events: events.slice(0, 3).map(e => ({
        home: e.homeName,
        away: e.awayName,
        homeNorm: normalizeTeamName(e.homeName),
        awayNorm: normalizeTeamName(e.awayName),
      })),
    });
    return null;
  }
