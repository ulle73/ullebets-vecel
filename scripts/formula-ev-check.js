...
const INCLUDE_DB = CLI_ARGS["no-db"] ? false : true;
...
async function loadBacktestsFromDb() {
  if (!INCLUDE_DB) return [];
  try {
    mongoClient = await clientPromise;
    const db = mongoClient.db(process.env.MONGODB_DB || "app");
    const docs = await db.collection("unibet-backtest").find({}).toArray();
    return docs.map((doc) => ({
      lines: Array.isArray(doc.lines) ? doc.lines : [],
      timestamp: toTimestamp(doc.generatedAt || doc.matchDate),
    }));
  } catch (err) {
    console.warn(
      "Skipping Mongo load for unibet-backtest:",
      err?.message || err
    );
    return [];
  }
...
function ingestLines({
  ...
  for (const bet of lines) {
    const win =
      bet.win != null
        ? bet.win
        : bet.evDetails && bet.evDetails.win != null
        ? bet.evDetails.win
        : null;
...
async function runGridSearch() {
  ...
  const dbBacktests = await loadBacktestsFromDb();
  for (const entry of dbBacktests) {
    ingestLines({
      lines: entry.lines,
