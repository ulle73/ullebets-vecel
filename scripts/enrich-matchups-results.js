...
// --- NYTT: imports för daterange-kedjekörning (självspawn) ---
...
async function main() {
  ...
  // --- NYTT: om --date är ett spann, kör samma script per dag och avsluta ---
  const _dateArgRaw = resolveDateArg();
  const _rangeRe = /^\d{4}-\d{2}-\d{2}-\d{4}-\d{2}-\d{2}$/;
  if (_rangeRe.test(_dateArgRaw)) {
    const dates = expandDateRange(_dateArgRaw);
    const selfPath = fileURLToPath(import.meta.url);
    for (const d of dates) {
      const res = spawnSync(process.execPath, [selfPath, `--date=${d}`], {
        stdio: "inherit",
        env: process.env,
      });
      if (res.status !== 0) {
    ...
    return; // klart för ranges, hoppa över singeldatumsflödet nedan
  }
  // -------------------------------------------------------------------------
