...
//       );
//       canFallbackToBroadMatch = false; // uniqueTournament-id fanns; respektera utfallet
//     }
//   }
...
// // ---------- API-funktioner ------------------------------------
// // async function fetchMatchIdsSofascore(page, team, ligaFilter, ligaSlug) {
// //   const norm = (s) => (s || "").toLowerCase().replace(/\s|-/g, ""); // ta bort mellanslag och bindestreck
...
//       payload.matches,
//       payload.data, // ibland direkt en array
//     ];
...
//     // fallback till RapidAPI-sofascore om tomt
...
//   // ---- Om RapidAPI inte gav något användbart: fallback till SofaScore-browser (som tidigare) ----
//   for (let sida = 0; ; sida++) {
...
//     if (!befintlig) {
//       // Skapa INTE nya poster om vi kör i selektivt läge utan basdata
...
//       if (creatingPartial) {
//         continue; // hoppa över, vi vill endast uppdatera
//       }
...
//   // Läs från ALLA kandidater (vi sammanfogar)
...
//   // Enskild extraflagga → tvinga backfill upp till antalMatcher (eller behåll större --backfill=N)
...
//         const forceUpdateBecauseSelective = modeLimited; // i selektivt läge försöker vi uppdatera
//         if (
//           ts <= teamMaxTs &&
//           !needsEnrichment &&
...
//             );
//             backfilledOlder++; // fortsätt utan continue – hämta & spara
//           } else {
//             console.log(
...
//             team.name,
//             existingRecordInfo.matchType, // samma sida som originalposten
//             [partial]
//           );
...
//           }
//           await sleep(1_000);
//           continue; // klar med denna match i odds-only-läge
//         }
...
//         if (!fetched && !plan.odds) continue; // inget att spara
...
//         // Spara endast om vi antingen har basdata (nya eller uppdateringar) eller om posten redan finns
//         // (för att undvika att skapa halvfyllda nya poster i selektivt läge).
//         const canSaveTeam =
...
//         // avgör home/away utifrån data eller befintlig post
//         let isHome = false;
...
//         // skriv även motståndarens fil om vi har namn/ID eller kan läsa dem från befintlig post
//         let oppName = null;
//         let oppType = null;
...
//           if (effectiveBackfill > 0) {
//             console.log("⏭️ Redan sparad – fortsätter (backfill aktiv)");
//             continue; // leta vidare efter äldre luckor
//           }
//           console.log(
