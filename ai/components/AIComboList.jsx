      ...
      {combos.map((combo, index) => {
        ...
        return (
          <article
            key={combo.id || `${index}-${combo.odds}`}
            className={clsx(
              "overflow-hidden rounded-2xl bg-[#020617] transition-all duration-300",
              borderColor,
              glowShadow
            )}
          >
                    ...
                    <div key={`${combo.id}-${label}`} className="space-y-2">
                      {urls.map((url, idx) => (
                        <a
                          ...
                          rel="noreferrer noopener"
                          className={clsx(
                            `group relative block w-full overflow-hidden rounded-full border ${buttonBorderColor} bg-transparent py-3 text-center text-sm font-bold transition-all ${buttonHoverGlow}`,
                            {
