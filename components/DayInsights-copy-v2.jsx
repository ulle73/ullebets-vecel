...
import {
  ...
  SCOPE_HOME,
  SCOPE_TOTAL,
} from "@/lib/statForecast/formulas";
import { STAT_CONFIG } from "@/lib/statForecast/statConfig";
      ...
      <div className="min-w-0">
        ...
        <div className="mt-2 space-y-1 text-xs text-gray-600">
          ...
          <div className="mt-1">
            ...
            {forecastParts.length ? (
              <div className="mt-2 rounded border border-dashed border-gray-200 px-2 py-1 text-[10px] text-gray-600">
                <div className="font-semibold text-gray-700">Prognos</div>
                <div>{forecastParts.join(" · ")}</div>
                {forecastSampleSize ? (
                  <div className="mt-0.5 text-[9px] text-gray-500">Aktiva drivare: {forecastSampleSize}</div>
                ) : null}
              </div>
            ) : null}
            {/* {mode === "under" && r.leagueMax ? (
      ...
      for (const { key: statKey, label } of STATS_FOR_VIEW) {
        for (const { value: periodKey } of PERIODS) {
          ...
          if (![hf, ha, af, aa].every(Number.isFinite)) continue;
            ...
            const forecastPerScope = Object.fromEntries(
              Object.entries(FORECAST_SCOPE_MAP).map(([scopeKey, bundleScope]) => {
